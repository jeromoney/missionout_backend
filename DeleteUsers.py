...
def get_apple_users():
    list_users = auth.list_users()
    return [user for user in list_users.users if
            len(user.provider_data) == 1 and user.provider_data[0].provider_id == 'apple.com']


def get_google_test_users():
    list_users = auth.list_users()
    return [user for user in list_users.users if re.search("[a-z]*\.[0-9]{5}@gmail.com", user.email) is not None]


def get_email_users():
    list_users = auth.list_users()
    return [user for user in list_users.users if
            len(user.provider_data) == 1 and user.provider_data[0].provider_id == "password"]
